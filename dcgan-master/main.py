...
flags.DEFINE_float("learning_rate", 0.0002, "Learning rate of for adam [0.0002]")  # 防止学习速度过快
...
flags.DEFINE_string("checkpoint_dir", "checkpoint", "Directory name to save the checkpoints [checkpoint]")  # 建立文件夹
flags.DEFINE_string("sample_dir", "samples", "Directory name to save the image samples [samples]")  # sample是建立的文件夹
flags.DEFINE_boolean("is_train", False, "True for training, False for testing [False]")  # 是否作为训练集合,主要是使用BN
flags.DEFINE_boolean("is_crop", True, "True for training, False for testing [False]")  # 在64张图片小样本中我们进行crop
...
def main(_):
    ...
    tl.files.exists_or_mkdir(FLAGS.checkpoint_dir)  # 如果当前目录系存在checkpoint文件夹则返回True;反之如果当前文件夹不存在
    # checkpoint文件夹则首先建立文件夹然后返回false
    ...
    with tf.device("/gpu:1"):  # 假设我们使用GPU 1
        ...
        net_d2, d2_logits = discriminator_simplified_api(real_images, is_train=True, reuse=True)  # net_d2 真正图像网络
        # sample_z --> generator for evaluation, set is_train to False
        # so that BatchNormLayer behave differently
        # 第四步, 利用上面训练好的G,D网络, 生成我们想要生成的图像. 测试网络不使用BN, 使用参数共享,reuse=True表示使用上面训练好的参数模型
        ...
        d_loss_real = tl.cost.sigmoid_cross_entropy(d2_logits, tf.ones_like(d2_logits), name='dreal')  # 这是是计算
        # 真实图像的损失,因为真实数据是1,所以我们使用ones_like产生1数据计算loss
        # discriminator: images from generator (fake) are labelled as 0
        # let `x = logits`, `z = labels`.  The logistic loss is: z * -log(sigmoid(x)) + (1 - z) * -log(1 - sigmoid(x))
        # 从这里我们可以清楚解决之前的疑惑.第一为什么我们在定义D判断网络需要在网络通过sigmoid函数之前返回一个logits,原因就是我们在利用
        # sigmoid_cross_entropy本身在调用内部函数时候就需要通过sigmoid函数了,所以
        # 我们相当于提前截断他.
        d_loss_fake = tl.cost.sigmoid_cross_entropy(d_logits, tf.zeros_like(d_logits), name='dfake')  # 这里我们对
        # G网络产生的虚假图像计算损失
        d_loss = d_loss_real + d_loss_fake  # D网络的损失判断真实图像的损失加上判断虚假图像损失之和
        # generator: try to make the the fake images look real (1), 也就是从G网路的角度来看,我们想要通过D网络更大可能判断为1
        # 这样G网络损失越小
    ...
    data_files = glob(os.path.join("./data", FLAGS.dataset, "*.jpg"))  # ./表示在最外外面的目录创建一个data文件夹,
    # 所以最后是data/celebA/*.jpg
    # glob 是或得当前匹配的内容,是一个list, 那门这么说,data_files里面存放的全面的图片路径, 每一张图片都有一个路径,
    # 最后是一起变成了一个list存放起来了
    ...
    for epoch in range(FLAGS.epoch):  # 总共迭代25次
        ## shuffle data
        shuffle(data_files)  # 首先把我们得到每一个图片路径进行打散,这样我们shuffle他,就相当于我们现在随机得到了文件路径,
        # 间接的打散文件了,要不然我们每次取64张图片还不重复了
        ...
        # 列表生成器,每一次取一个路径,然后得到一副图像,然后最后形成64张图像
        # 原图像是image_size:108,为了进行匹配我们在这里进行center_crop将图像中心裁剪成64*64大小
        sample_images = np.array(sample).astype(np.float32)  # 因为get_image返回的是np.array这里的话我们是将列表变成了
        # np.array然后在进行类型转换变成np.float32类型了
        ...
        # 首先第一点有点不明白为什么来一个inf, 最后结果图像张数总数/batch_size(64) 这里使用批量梯度下降法?
        #  //地板除,结果都是整数
        for idx in range(0, batch_idxs):  # 每次都是取一个batch进行运算,使用batch 梯度下降法
            ...
            batch = [get_image(batch_file, FLAGS.image_size, is_crop=FLAGS.is_crop, resize_w=FLAGS.output_size, is_grayscale = 0)
                     for batch_file in batch_files]  # 中心裁剪成为64,64大小

            batch_images = np.array(batch).astype(np.float32)  # 每一轮都获得64张训练图像,总共多少轮就进行多少次batch,
            ...
            # 这是运行一个batch所需要的时间,而且记录一下D和G的loss
            ...
            if np.mod(iter_counter, FLAGS.sample_step) == 0:  # 所以的每隔500次保留一下G生成的图片
                ...
                # 保存的是8*8格式的图像,所以64我们使用sample_size和batch_size都是64就可以理解了
            ...
            if np.mod(iter_counter, FLAGS.save_step) == 0:  # 每隔500次保存一个网络的参数
                ...
                # 保存网络G的参数,路径是heckpoint/celebA_64_64/net_g.npz
                tl.files.save_npz(net_d.all_params, name=net_d_name, sess=sess)
                # 保存网络D的参数,路径是heckpoint/celebA_64_64/net_d.npz

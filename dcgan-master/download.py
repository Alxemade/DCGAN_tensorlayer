...
def download_celeb_a(dirpath):  # 下载celebA.zip文件,然后进行解压缩,
	data_dir = 'celebA'
	if os.path.exists(os.path.join(dirpath, data_dir)):  # 形成一个data/celebA目录
	...
	if os.path.exists(save_path):
		...
		download_file_from_google_drive(drive_id, save_path)  # 从openface 下载文件

	zip_dir = ''
	with zipfile.ZipFile(save_path) as zf:  # 将已经下载到本地文件进行解压缩,产生图片
		zip_dir = zf.namelist()[0]
		zf.extractall(dirpath)
	os.remove(save_path)  # 删除保存文件目录
	os.rename(os.path.join(dirpath, zip_dir), os.path.join(dirpath, data_dir))  # 将当前文件命名成'celebA'
...
def prepare_data_dir(path = './data'):   # 创建一个data目录
    if not os.path.exists(path):
        os.mkdir(path)

if __name__ == '__main__':
    args = parser.parse_args()  # 命令行解释器
    prepare_data_dir()
